...
| 무신사 3진 분류 기준 | "1–2 / 3 / 4–5" vs "0 / 1–4 / 5" | **0점 / 1–4점 / 5점** |
| 냐암냐암 스택 | "HTML/CSS" vs "PHP + MySQL" | **PHP + MySQL** |

### 출처 주의 — 레포에 없는 원본

아래 발표자료는 **git에 추적되지 않으며 현재 레포에도 없다.**
수치를 재검증할 수 없으므로, 이 파일의 기록을 유일한 근거로 삼는다.

| 파일 | 근거가 되는 항목 | 상태 |
|---|---|---|
| `ML_project_최종2.pdf` | 무신사 전 항목 (5진 36.15% / 2진 71.53% / 3진 84.62% 등) | **레포 없음.** 사용자 확인 (2026-08-27) |
| `D조_발표_6월7일.pdf` | 냐암냐암 3인 팀 · 담당 · PHP+MySQL | **레포 없음.** 사용자 확인 (2026-08-27) |
| `YORR 최종 발표.pdf` | YORR (참조용) | 레포에 있으나 git 미추적. **페이지 번호 매핑이 어긋남 — §6 참조** |

두 PDF를 `docs/`에 넣어두면 다음 세션이 대조할 수 있다. 지금은 불가능하다.
...
> 출처: `ML_project_최종2.pdf` (레포 미포함 — 아래 §0 출처 주의 참조)

- **6인 팀 프로젝트.** 본인 담당은 **데이터 수집 · EDA · 모델링**
...
- **분류 방식을 세 가지 시도한 뒤 3진을 채택했다** (이 탐색 과정이 성과다)
  ...
  | 5진 분류 | **36.15%** | 7p |
  | 2진 분류 | 평균 **71.53%** | 48p |
  | **3진 분류 (0점 / 1–4점 / 5점)** | 평균 **84.62%** ← 채택 | 39p |
...
- **냐암냐암 (Nyam Nyam)** — 캡스톤디자인 1, **3인 팀 · 본인 조장**.
  ...
  스택은 **PHP + MySQL** (기존 사이트 카드에 PHP가 누락돼 있었다).
  출처: `D조_발표_6월7일.pdf` — 단 **조장 여부는 발표자료에 표기가 없고
  본인 확인으로 확정**했다
...
> **해소됨 (2026-08-27)** — 무신사 프로젝트 팀 규모·정확도·분류 방식,
> 냐암냐암 팀 규모·담당·스택. `ML_project_최종2.pdf`·`D조_발표_6월7일.pdf`
> 대조로 확정. §2에 반영했다. 단 **두 PDF는 레포에 없다** — §0 「출처 주의」 참조.
> 냐암냐암의 **조장 여부만 발표자료에 표기가 없고 본인 확인으로 확정**했다.
