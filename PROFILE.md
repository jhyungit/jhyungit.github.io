# PROFILE.md — 사실 원본

이 문서는 **모든 취업 산출물의 단일 사실 원본(single source of truth)**이다.

**적용 대상**
1. 이력서 PDF (`docs/resume.html`)
2. 포트폴리오 PDF (`docs/portfolio.html`)
3. GitHub 프로필 README (`jhyungit/jhyungit`)
4. 포트폴리오 웹사이트 (`src/data/`, https://jhyungit.github.io/)

**여기에 없는 것은 어디에도 쓰지 않는다.**

> **산출물 역할 분담 — 모든 프로젝트가 모든 문서에 들어가지 않는다.**
> `docs/resume.html`은 **3페이지 백엔드 요약**이라 프로젝트를 선별해 싣는다
> (YORR · TripCrew · yorr-arcade · IBK · 달리셔스). `docs/portfolio.html`은
> 전체 근거를 싣는 자리다. **무신사·냐암냐암이 이력서에 없는 것은 누락이 아니라
> 의도된 것이다 — 이력서에 추가하지 않는다.**

수치는 확정값이다. 추정·반올림·재해석하지 않는다.

---

## 0. 동기화 규칙

**동기화 방향은 단방향이다.**

```
PROFILE.md  (원본)
    ↓
docs/resume.html
    ↓
docs/portfolio.html
    ↓
src/data/            (웹사이트)
    ↓
GitHub 프로필 README (jhyungit/jhyungit)
```

- 사실이 바뀌면 **PROFILE.md를 먼저 고친다.** 그다음 위 순서로 전파한다
- **역방향으로 고치지 않는다.** 산출물에서 발견한 새 사실은 먼저 PROFILE.md에
  반영한 뒤 나머지에 전파한다
- 산출물 사이에 불일치가 발견되면, 어느 쪽이 맞는지 판단하지 말고
  **PROFILE.md를 기준으로 양쪽을 맞춘다.** PROFILE.md에 없으면 사용자에게 묻는다

### PDF 렌더링 — HTML을 고쳤다면 PDF도 다시 만들어야 한다

`docs/*.html`은 **소스**이고 배포되는 것은 `public/*.pdf`다. HTML만 고치면
사이트 방문자는 여전히 낡은 PDF를 받는다.

| PDF (배포됨, **git 추적**) | 소스 HTML | 페이지 |
|---|---|---|
| `public/resume.pdf` — 사이트 `/resume.pdf`로 링크됨 | `docs/resume.html` | **3p** |
| `public/portfolio.pdf` — **사이트 링크 없음** | `docs/portfolio.html` | **4p** |

- 렌더러는 **WeasyPrint 69.0**이다. 다른 렌더러(headless Chrome 등)로 바꾸면
  페이지네이션이 달라진다
- `dist/*.pdf`는 빌드가 `public/`에서 복사한 것이다. 직접 만들지 않는다
- `docs/`의 PDF 3개(`ML_project_최종2.pdf` 등)는 **원본 발표자료 입력물**이며
  HTML에서 생성되지 않는다. 덮어쓰지 않는다
- ⚠️ **`docs/*.html`은 `@font-face`가 없어 Pretendard를 시스템 폰트로만
  찾는다.** Pretendard가 없는 환경에서 렌더링하면 자폭·행간이 달라져
  **3페이지 이력서가 4페이지로 넘어간다.** 렌더링 전에 폰트 설치를 확인한다
  (2순위 `Noto Sans CJK KR`도 `Noto Sans KR`과 다른 패밀리명이라 대체되지 않는다)

> ⚠️ **현재 `public/*.pdf`는 낡았다 (2026-09-03).**
> 수상 표기 정정(§6 2026-09-03 항목)이 HTML·`src/`에는 반영됐지만
> PDF는 재렌더링하지 못했다 — 이 Windows 머신에 WeasyPrint·GTK·Pretendard가
> 없다. 배포된 `/resume.pdf`에는 아직 **「공통 프로젝트 최종 1위」·
> 「우수상 (7팀 중 2위)」**가 남아 있다. **면접 전에 반드시 재렌더링한다.**

### 과거에 실제로 발생한 불일치 (재발 방지용 기록)

| 항목 | 발생한 표기 차이 | 확정값 |
|---|---|---|
| WebSocket 이벤트 스키마 | "28종" vs "30여 종" | **28종** |
| yorr-arcade 팀 서비스 | "채택" vs "채택·이식" | **채택은 내 것 / 이식은 팀원** |
| YORR 배포 주소 | yorr.site vs vercel 주소 | **yorr.site** |
| TripCrew 실시간 구조 | "Redis 기반" vs "SimpleBroker" | **인메모리 SimpleBroker** |
| 달리셔스 팀 규모 | "5인 팀" vs "6인 팀" | **6인 팀** |
| 수상 건수 | "2건" vs "4건" | **4건** |
| 무신사 정확도 | "84%" vs "84.62%" | **84.62%** (RF 교차검증 평균) |
| 무신사 팀 규모 | 미기재(1인으로 읽힘) vs 6인 | **6인 팀** |
| 무신사 3진 분류 기준 | "1–2 / 3 / 4–5" vs "0 / 1–4 / 5" | **0점 / 1–4점 / 5점** (25p 원본 표) |
| 무신사 EDA 편중 축 | "1점·5점 양극화" vs "0점 편중" | **0점(리뷰 없음) 편중** (23p) |
| 무신사 2진 정확도 | 48p "71.54%" vs 50p "71.53%" | **71.53%** (50p 종합표 기준) |
| 무신사 명칭 | "별점 예측" vs "평점 예측" | **평점 예측** (원본 표지) |
| 냐암냐암 스택 | "HTML/CSS" vs "PHP + MySQL" | **PHP + MySQL** |
| YORR 소셜 로그인 | "Google 단독·본인 담당" vs "Google / Kakao·팀원 담당" | **Google / Kakao · 팀원 구현** |
| SSAFY 공통 수상 명칭 | "최우수상" / "최종 1위" / "1위" | **우수상 (서울 4반 1등)** — 상장 원본 |
| SSAFY 관통 수상 분모 | "7팀 중 2위" | **서울 11반 2등** — 팀 수는 상장에 없음 |

### 출처 주의 — 원본 발표자료의 소재와 대조 가능 여부

아래 발표자료는 **git에 추적되지 않지만 `docs/` 아래에 실제로 존재한다.**
다음 세션은 기억으로 채우지 말고 **파일을 열어 대조한다.**

| 파일 | 근거가 되는 항목 | 상태 |
|---|---|---|
| `docs/ML_project_최종2.pdf` | 무신사 전 항목 (5진 36.15% / 2진 71.53% / 3진 84.62% 등) | **레포에 존재. 대조 가능.** 전 항목 재대조 완료 (2026-08-27) |
| `docs/D조_발표_6월7일.pdf` | 냐암냐암 3인 팀 · 담당 · PHP | **레포에 존재. 대조 가능.** 재대조 완료 (2026-08-27) |
| `docs/YORR 최종 발표.pdf` | YORR (참조용) | 레포에 존재하나 **페이지 번호 매핑이 어긋남 — §6 참조** |

**PDF에 텍스트 추출 도구가 없다** (`pdftotext`·`pymupdf`·`pdfminer` 모두 미설치).
2026-08-27 재대조는 FlateDecode 스트림과 `/ToUnicode` CMap을 직접 파싱하는
임시 스크립트로 수행했다. 같은 방법을 다시 쓰거나 `brew install poppler`로
`pdftotext`를 설치한다.

> **차트·스크린샷 안의 글자는 추출되지 않는다.** 아래 항목은 이미지에만 있어
> 텍스트로 재대조하지 못했고 **사용자 확인을 근거로 유지**한다:
> 무신사 — 피처 중요도 2위(가격), DevTools Network 탭 추적 서술, 본인 역할 분담 /
> 냐암냐암 — MySQL, 조장 여부, 역할↔이름 매핑

최종 확인: 2026-08-27 (발표자료 원문 대조)

---

## 1. 기본 정보

| 항목 | 값 |
|---|---|
| 이름 | 이정현 (Jeonghyun Lee) |
| 직군 표기 | **Backend Developer** (단일 표기. "Fullstack", "Data-driven" 병기 금지) |
| 한 줄 소개 | Spring Boot 기반 API 서버를 설계하고, 쿼리·인덱스로 응답 속도를 개선하는 백엔드 개발자입니다. |
| 이메일 | jh021199@gmail.com |
| GitHub | https://github.com/jhyungit |
| 웹사이트 | https://jhyungit.github.io/ |
| 위치 | Seoul, Korea |

> 전화번호는 이 파일에 두지 않는다. `docs/resume.html`(git 미추적)에만 있다.

---

## 2. 확정 수치 — 반올림·추정 금지

### TripCrew (2026.04–06, SSAFY 관통 프로젝트, 2인 팀, 팀장, 백엔드 담당 기여도 50%)
- 30만 건 기준 **0.473초 → 0.018초 (약 26배)**
- 인덱스: **(role, created_at) 복합 인덱스**
- EXPLAIN: **type ALL → ref**, **filesort 제거**
- 인덱스 적용 후에도 **OFFSET 10만 구간 0.244초** ← 빼면 안 된다
  → 대용량 페이징의 근본 해법은 커서 방식
- 낙관적 락: version 컬럼, 충돌 시 **409**
  (충돌이 드문 특성상 비관적 락 대비 성능 유리하다 판단)
- Flyway **V1~V17**
- Docker Compose **4컨테이너** (MySQL · Redis · Backend · Caddy)
- Caddy 리버스 프록시로 HTTPS 운영
- **SSAFY 1학기 관통 프로젝트 우수상 (서울 11반 2등)**
  - 상장 제목은 「1학기 프로젝트 우수상」, 순위는 상장 내용란의 부가 정보다
  - 수여일 **2026.06.26** · 증서번호 **2026-45-050066**
  - ⚠️ **팀 수(분모)는 상장에 없다. 「7팀 중」으로 쓰지 않는다**
- 실시간 공동 편집: **STOMP + 인메모리 SimpleBroker · 단일 인스턴스 구조**
- 인증: JWT + Refresh Token, **Kakao / Naver** 소셜 로그인 (OAuth 2.0)
- 스택: Java 17 · Spring Boot 3 · Spring Security · MyBatis · MySQL 8 ·
  Redis · Vue 3 · Docker Compose · AWS EC2 · JWT · Flyway
- Live: https://tripcrew.duckdns.org
- Code: https://github.com/tripcrew/tripcrew

### YORR (2026.07–08, SSAFY 공통 프로젝트, 6인 팀, PM/팀장)
- WebSocket 이벤트 스키마 **28종** 단독 정의
- 최고 동시접속 **80명**
- 누적 게임 참여 **635건** / 플레이 **487판**
- 만족도 설문 **n=17, 4점 이상 94.1%** (5점 58.8% / 4점 35.3%)
- 팀원 MR **21건** 통합
- 1차 배포 **2026.07.30** / 2차 배포 **2026.08.05**
- **SSAFY 2학기 공통 프로젝트 우수상 (서울 4반 1등)**
  - 상장 제목은 「우수상」, 순위는 상장 내용란(`[2학기]공통 프로젝트 서울 4반 1등`)의
    부가 정보다
  - 수여일 **2026.08.10** · 증서번호 **2026-45-050195**
  - ⚠️ **「최우수상」·「최종 1위」로 쓰지 않는다. 상장에 없는 표현이다**
- 게임 라인업 **3종** (그중 2종이 yorr-arcade 출신)
- **12라운드** 종료 판정
- 소셜 로그인: **Google / Kakao** (OAuth 2.0, 팀원 구현)
- 라운드 상태 저장소: **InMemory** (`InMemoryRoundStateStore`)
- WebSocket: **raw handler** (`GameWebSocketHandler`)
- 음성: **WebRTC 풀메시 P2P**. 서버는 ICE 후보 교환만 중계하고 음성 트래픽은 경유하지 않음
- **coturn TURN 서버** — NAT 환경에서 P2P 직결 실패 시에만 중계
  (근거: `docs/yorr-evidence.md` 「음성을 서버에 태우지 않은 이유」 및 `yorr-architecture.svg`)
- 스택: Java · Spring Boot · JPA · WebSocket · WebRTC · coturn · MySQL ·
  Docker · AWS EC2 · Jira · GitLab
- Live: https://yorr.site
- Code: https://github.com/team-yorr/yorr

### 달리셔스 (2023.01–03, T-academy 기업 연계, 6인 팀, CB 모델 담당 & 팀 리딩)
- **6인 팀**
- **6팀 중 1위** (기업 연계 최종 프로젝트 우수상)
- 데이터 **약 5만 건**, **사내 제공** (크롤링 아님 — 명시할 것)
- 운영팀 수동 메뉴 선정 **30분 → 3분 (약 90% 단축)**
- 잠재고객 **2만 명** 규모 고객사 계약 성사 기여
- Cosine Similarity 기반 Content-Based 모델
  (CB · CF · ML · DL 결합 하이브리드로 고도화)
- 한 번의 주문 기록만으로 개인화 추천 → 콜드 스타트 해결
- 스택: Python · Pandas · Scikit-learn · PySpark · Cosine Similarity
- Code: https://github.com/jhyungit/Final_project
- 동아일보 보도: https://www.donga.com/news/It/article/all/20230503/119128322/1

### 무신사 향수 평점 예측 (T-academy ASAC 과정, **6인 팀**)
> 출처: `docs/ML_project_최종2.pdf` (총 64슬라이드, 레포에 존재 — §0 출처 주의 참조)

- **명칭은 「평점」으로 통일한다.** 원본 표지가
  `MUSINSA PERFUME CATEGORY 평점 예측`, 본문 표제가 `향수상품 평점 RATIO 예측`이다.
  **「별점」으로 쓰지 않는다.** 단 원본도 데이터 컬럼을 가리킬 때는 「별점」을
  혼용한다 — 프로젝트 이름에는 평점을 쓴다
- **6인 팀 프로젝트.** 표지 공동저자 6인
  (한준표 · 임유경 · 이찬영 · 안가영 · 안승아 · 이정현)으로 확인
  (팀 규모가 누락되면 1인 프로젝트로 읽힌다. 반드시 병기한다)
- **담당 표기는 이 문장을 그대로 쓴다 —
  「데이터 수집(크롤러 구현)·EDA 담당, 모델링 공동 수행」**

  | 항목 | 범위 |
  |---|---|
  | 데이터 수집 | **본인이 크롤러 코드 작성** |
  | EDA | **본인 수행.** 평점 분포 편중을 발견한 것이 본인 |
  | 모델링 | **6인 공동 수행.** 본인 단독 아님 |

  > ⚠️ **출처는 발표자료가 아니라 「본인 확인」(2026-08-27)이다.**
  > 발표자료에는 역할 분담 슬라이드가 없다. 이 경계를 넓히지 않는다.
  > 특히 **모델링을 단독 성과로 쓰지 않는다** — 정확도 84.62%는 팀 성과다
- HTML 파싱으로 수집 불가한 리뷰 데이터 → "사용자 동작 시점에 비동기 로딩"
  가설 수립 → **브라우저 DevTools Network 탭으로 HTTP 요청 패턴 추적** →
  요청 API 특정 → 코드로 재현·안정적 수집
- **EDA 확정 문장 — 자소서·이력서·포트폴리오에 이 문장을 그대로 쓴다:**

  > **리뷰가 없는 상품이 0점으로 처리되어 0점과 5점에 분포가 몰림**

  근거(23p): 0.0 → **800건** / 5.0 → **337건** / 4.9 → 114건.
  결측 평점을 0으로 채운 처리 때문에 0점이 최빈값이 됐다.
  - **"1점·5점 양극화"로 쓰지 않는다.** 편중의 한 축은 1점이 아니라
    **0점(리뷰 없음)** 이다. 이걸 틀리면 "1점이 왜 많았나"에 답할 수 없다
  - 이 분포가 **3진 기준을 `0점 / 1–4점 / 5점`으로 잡은 유일한 근거**다.
    "왜 0점을 별도 구간으로 두었나"는 이 문장으로만 설명된다 —
    결측 처리 판단을 설명하는 소재로 쓴다
- **분류 기준 원본 표 (25p)** — 이 표가 유일한 기준이다

  | 별점 | 2진 분류 | 3진 분류 |
  |---|---|---|
  | **0점 (리뷰 없음)** | **제외(X)** | 포함 |
  | **1~4점** | 포함 | 포함 |
  | **5점** | 포함 | 포함 |

  즉 **2진은 0점 행을 버리고 1–4점 / 5점으로 나눈 것**이다.
  "고평점 / 저평점"으로 뭉뚱그리면 0점 제외 사실이 사라진다
- **분류 방식을 세 가지 시도한 뒤 3진을 채택했다** (이 탐색 과정이 성과다)
  | 분류 | RF 정확도 | 근거 |
  |---|---|---|
  | 5진 분류 (1~5점) | **36.15%** | 7p (`RF 정확도 0.3615`) |
  | 2진 분류 (0점 제외) | **71.53%** | **50p 종합표** — 아래 주의 |
  | **3진 분류 (0점 / 1–4점 / 5점)** | 평균 **84.62%** ← 채택 | 39p · 46p 종합표 |
- 최종 정확도 **84.62%** (RF, 교차검증 5회 평균). GridSearch 후 **86.00%**
  - 39p 5회 검증값: 86.38 / 79.49 / 85.9 / 86.75 / 84.62 → 슬라이드 표기 평균 **84.62%**
- XGBoost: 평균 **84.96%**, GridSearch 후 **86.42%** (46p 종합표)

> ⚠️ **2진 71.53% — 원본 자체가 두 값으로 갈린다.**
> 48p 슬라이드는 `RF 평균 71.54%`, 50p 종합표는 `Baseline 71.53%`로 적혀 있다
> (5회 검증 71.68 / 68.14 / 74.11 / 71.43 / 72.32의 실제 평균은 71.536).
> **71.53%를 쓴다** — 3진 84.62%와 같은 종합표에서 나온 값이라 비교 축이 일치한다.
> 이 항목의 근거 페이지는 48p가 아니라 **50p**다.
- 가장 중요한 Feature는 **리뷰 수**, 그다음 **가격** (41p)
- 리뷰 긍부정 분류에 **Keras GRU** 사용, **72.36%**
- **네트워크 요청 패턴 분석은 이 프로젝트에만 해당**
- 스택: Python · Pandas · Scikit-learn · Random Forest · XGBoost ·
  Selenium · BeautifulSoup · Keras(GRU) · DevTools (Network)

> ⚠️ **"84%"로 쓰지 않는다.** 확정값은 **84.62%**다.
> 어느 수치를 쓰든 어떤 모델·어떤 조건인지 함께 밝힌다
> (RF 교차검증 평균 / GridSearch 후 / XGBoost 여부).

### yorr-arcade (2026.07–08, 개인 프로젝트, 1인 개발)
- React · TypeScript · Three.js · Socket.IO · Vite
- 게임 **2종** 1인 개발 — 3D 렌더링과 실시간 이벤트 동기화 직접 구현
- 팀 서비스에 채택되어 최종 라인업 3종 중 2종 차지
- **팀 서비스로의 이식은 팀원 담당**

### IBK기업은행 (2023.07–08, IT금융개발부 IT글로벌개발팀 청년인턴)
- **4개 앱** 통합 슈퍼앱 프로토타입 (i-ONE Bank, Box POS 등) 기획·구현, Figma
- 개인·기업 고객 특성을 반영한 Flowchart 로직 설계 (금융홈·포스홈 분리)
- 기술 로직을 비기술 이해관계자용 고객 경험 시나리오로 재구성해 발표
- IT본부장 참석 최종 발표에서 **우수팀 · 우수인턴** 선정

### 그 외 프로젝트
- **냐암냐암 (Nyam Nyam)** — 캡스톤디자인 1, **3인 팀 · 본인 조장**.
  담당: **코드 작성 · 데이터베이스 구축 · 발표**.
  스택은 **PHP + 관계형 DB**까지만 확정 (기존 사이트 카드에 PHP가 누락돼 있었다).
  출처: `docs/D조_발표_6월7일.pdf` (11슬라이드, 2026-08-27 재대조).
  - **발표자료로 확인됨**: 표지 공동저자 **3인** (이정현 · 김예은 · 김나연) /
    **PHP** / 회원가입 · 리뷰 · 별점 기능 / 10p 역할 슬라이드에
    `코드작성`·`데이터베이스 구축`·`PPT 작성 및 편집`·`발표자`
  - **본인 확인으로 확정** (발표자료에 근거 없음): **조장 여부**, 역할↔이름 매핑
  - ⚠️ **MySQL은 확정이 아니다.** 발표자료는 "데이터베이스"로만 표기한다.
    **「MySQL」로 쓰지 말고 「PHP + 관계형 DB」로 쓴다** → §6 미확인 참조
- **PySpark 마포구 카페 현황 분석** — 대용량 처리 · 시각화.
  https://github.com/jhyungit/PySpark-project
- **MLB 선수 연봉 예측** — **MLB Stats API** · Linear Regression.
  https://github.com/jhyungit/ML_baseball_project

### 교육 / 자격 / 수상
- SSAFY 15기 Java 전공반: 2026.01–2026.12, **1,725시간**, 11반 반장
- ASAC 빅데이터 분석가 과정 1기: 2022.09–2023.03, **920시간**, SK planet 주관
- 명지대학교 정보통신공학과: 2017.03–2022.02 졸업, **3.45 / 4.5**
- 호주: **2019.02–2020.01 (11개월)**. ILSC Sydney Intermediate → Advanced 수료,
  멜버른 현지 근무 (Waterfront Southgate, 2019.08–2019.12)
- 정보처리기사 **2024.12** (한국산업인력공단)
- SQLD **2025.12** (한국데이터산업진흥원)
- OPIc **IH** **2025.06** (ACTFL)
- **수상 4건**
  - SSAFY **2학기 공통 프로젝트 우수상 (서울 4반 1등)** — 2026.08.10
    (증서번호 2026-45-050195)
  - SSAFY **1학기 관통 프로젝트 우수상 (서울 11반 2등)** — 2026.06.26
    (증서번호 2026-45-050066)
  - IBK **우수팀 · 우수인턴** — 2023.08
  - ASAC 과정 **우수상 (1위)** — 2023.03

### 그 외 활동
- **사회인 야구** — 2021~현재 (5년째 진행 중)
  - 팀업 캠퍼스 일요 루키 리그 **우승**, **MVP 선정**
  - ⚠️ 이 MVP는 위 「수상 4건」에 **포함하지 않는다.**
    수상 4건은 학업·직무 관련 수상만 센다. 사이트의 `Awards 4`가 이 값과 묶여 있다
  - ⚠️ 개인 기록(타율 0.750 · OPS 2.100)은 **타석 수 미확인**이라
    사이트·이력서·포트폴리오·README에 쓰지 않는다.
    재활→복귀 서사가 있는 자소서에서만 쓴다
  - 리그명은 「팀업 캠퍼스 일요 루키 리그」로 전체를 쓴다.
    "사회인 야구 리그"로 줄이지 않는다 (급을 올린 표기가 된다)
  - ⚠️ **게재 범위: 사이트 About 섹션에만 쓴다.**
    `docs/resume.html`(3페이지 백엔드 요약)·`docs/portfolio.html`·
    GitHub README에는 넣지 않는다. §0 단방향 전파의 예외 항목이다

### 라이브 서비스
| 서비스 | URL | 비고 |
|---|---|---|
| YORR | https://yorr.site | AWS EC2 |
| TripCrew | https://tripcrew.duckdns.org | DuckDNS 무료 도메인 — 게시·면접 전 생존 확인 |

---

## 3. 기술 사실 — 혼동 잦음

| 항목 | 사실 | 흔한 오류 |
|---|---|---|
| YORR WebSocket | **raw handler** (`GameWebSocketHandler`) | STOMP로 착각 |
| TripCrew WebSocket | **STOMP** | — |
| YORR 라운드 상태 저장소 | **InMemory** (`InMemoryRoundStateStore`) | "Redis로 라운드 상태 관리" |
| TripCrew 실시간 협업 | **인메모리 SimpleBroker · 단일 인스턴스** | "Redis Pub/Sub" ← 실제 발생 |
| YORR 소셜 로그인 | **Google / Kakao** (팀원 구현) | Google 단독으로 기재 / 본인 성과로 기재 |
| TripCrew 소셜 로그인 | **Kakao / Naver** | — |
| 네트워크 요청 패턴 분석 | **무신사만** | MLB·달리셔스에 전용 금지 |
| MLB 데이터 수집 | **MLB Stats API** | "크롤링" 표기 금지 |
| 달리셔스 데이터 | **사내 제공** | "크롤링" 오해 금지 |
| YORR 게임별 Redis 스토어 | 존재하나 **담당자 미확인** | 기재하지 않음 |

---

## 4. YORR 담당 경계 — 팀 프로젝트

> 이 절의 목적은 **팀 레포에 코드가 있다는 것과 내가 그것을 만들었다는 것을
> 구분**하는 데 있다. 산출물에 들어가는 문장은 아래 "담당 확정" 범위를 넘지 않는다.
>
> 분석 방법: git log 기반, 머지 커밋 제외, lock/build/dist/node_modules 제외
> 역할: PM/팀장 · 실시간 통신 설계 · 운영/장애 대응

### 담당 확정 — 커밋 근거 있음

**(1) WebSocket 통신 계약 설계 — 대표 소재**
- `GameWebSocketHandler.java` (최다 수정)
- `WsEnvelope.java`, `WsErrorCode.java`, `RoomSessionRegistry.java`,
  `WebSocketConfig.java`
- `ws/dto/` 하위 페이로드 클래스 (요청-응답 짝 구조)

설계 포인트:
- record + 제네릭 P — 페이로드 타입을 컴파일 타임에 보장, 봉투 구조는 단일화
- `@JsonInclude(NON_NULL)` — null 필드를 JSON에서 제외해 프론트 계약의
  optional과 일치
- `withMsgId()` — 요청 msgId를 응답에 되돌려 요청-응답 상관관계(correlation) 확립
- `withRoomId()` — 입장 전후 스코프 변화를 봉투 레벨에서 표현
- 불변 + 정적 팩토리 `of()` — 서버 시각 자동 주입
- 효과: 게임 3종 확장 시 규격 위에 얹기만 하면 됨

**(2) WebRTC 음성 시그널링**
- `VoiceIceService.java`, `VoiceIceController.java`
- `VoiceSignalPayload`, `VoiceSignaledPayload`, `VoicePeersPayload`
- `RoomSessionRegistryVoiceTest.java` (단위 테스트)
- 게임 상태는 WebSocket 서버 경유, 음성은 P2P로 분리.
  ICE 후보 교환만 서버가 중계
- **coturn TURN 서버 연동** — NAT 환경 P2P 직결 실패 시에만 중계.
  풀메시 구조이므로 인원이 커지면 SFU 도입이 필요하다는 한계까지 파악
- CORS·에코 문제를 실사용 중 발견해 수정

**(3) 라운드 동기화 · 타이머**
- `RoundTimerService`, `RoundSynchronizationService`, `RoundState`,
  `RoundStateStore`(port), `InMemoryRoundStateStore`, `GameCompletionService`
- 12라운드 종료 판정 로직

**(4) 팀장으로서 통합·형상 관리**
- Git 협업 컨벤션 정립 (`CONTRIBUTING.md`), MR 템플릿 수립
- 팀원 MR develop 통합 **21건**
- v0.1 릴리스 머지 관리
- 잘못된 main 머지 revert 대응

**(5) 그 외**
- `RoomValidationService`, `RoomValidationController`, `RoomCreateService`
- QA 이슈: 세션 토큰 만료 시 대기실 연결 화면 멈춤 (우선순위 상위, 완료)
  → 세션↔playerId 매핑과 SESSION_EXPIRED 구분 처리로 해결
- 관전 화면이 조작보다 먼저 결과를 노출하던 문제
  → 화면 단위 수정이 아니라 shake/shaken 이벤트 계약을 신설해 해결.
    동일 유형의 순서 역전을 구조적으로 차단
- 클라이언트 신뢰 경계 분리 — 센서 값 전량 전송은 그 자체가 지연이 되므로
  동작 판정은 클라이언트에 두되, 상태·순서·점수 갱신은 서버 이벤트로만
  가능하도록 계약을 설계
- 사용자 피드백을 백로그화해 후속 릴리스에 반영
  (튜토리얼·툴팁, 센서 보정·버튼 조작 지원)

**(6) AI 개발 워크플로 · 팀 리딩**
- CLAUDE.md에 코딩 규칙을, CONTRIBUTING.md에 커밋·MR 컨벤션을 고정해
  6인이 같은 형태의 결과를 얻도록 표준화
- MCP로 Jira·Notion을 연동해 티켓 발급·스프린트 운영·요구사항 문서화 자동화
- 생성 결과는 실행계획과 커밋 단위로 확인한 뒤 반영
- Jira 이슈·일정·릴리스 관리

### 담당 아님 — 내 성과로 쓰지 말 것

| 소재 | 실제 담당 |
|---|---|
| Redis 스냅샷 세션 재접속 **구현** | 팀원 |
| 게임 모듈 추상화 (WebSocket 입력 + Redis 상태 분리) | 팀원 |
| Micrometer 커스텀 지표 | 팀원 (Infra) |
| CI/CD (Jenkinsfile, deploy/) | 팀원 (Infra) |
| Prometheus·Grafana 모니터링 구축 | 팀원 (Infra) |
| Expectimax 요트 AI | 팀원 (AI/Backend) |
| 게임 재접속 404 버그 | 팀원 |
| 소셜 로그인 (Google · Kakao) | 팀원 (고용훈 · yonghun) |

### 반쪽만 사실인 것 — 정확히 구분할 것

- **재접속**: 프로토콜 설계(`SysReconnectPayload`/`SysReconnectedPayload` DTO)는
  내 것. 복구 로직 구현은 팀원.
  → "규격을 정의했고 구현은 팀원이 맡았다"까지만 기재
- **모니터링 도구 세팅 브랜치 머지**: 내가 머지했으나 구현은 팀원.
  머지는 통합이지 구현이 아님
- **yorr-arcade 게임 2종**: 개인 레포에서 1인 개발한 것은 내 것이고,
  팀 서비스로 채택된 것도 사실. 단 **팀 서비스로의 이식은 팀원 담당**

### 기재 규칙
- **코드 라인 수는 쓰지 않는다.** 담당 모듈 기준으로 서술한다
- 규격 설계와 통합을 맡는 것은 팀장의 정상적인 역할 분담이다

---

## 5. 표현 규칙

### 쓰지 않을 표현
- 형용사 자기평가: "성실한", "긍정적인", "열정적인", "Hard worker",
  "Fast learner", "Reliable teammate", "건강한 개발자"
- 자기평가 숙련도 척도 (별점 5점 등) — 검증 불가
- 학점을 성과로 제시 ("A0 성적 취득")
- 성과가 아닌 것을 배지로 ("3개월", "2개월")
- 직군 병기: "Backend / Fullstack", "Backend · Data-driven"
- 데이터/ML 이력을 **앞에** 배치 — "백엔드로 넘어온 데이터 사람"으로 읽힌다.
  백엔드 뒤에 두고, 서사는 About/Hero 마지막 한 줄로

### 쓸 표현
- 검증 가능한 수치 + 검증 방법 ("EXPLAIN으로 type ALL→ref 확인")
- 개선의 한계까지 ("26배 개선했고, OFFSET 10만 구간은 0.244초로 남는다")
- 담당 경계 명시 ("규격은 내가, 구현은 팀원")
- 의사결정 근거와 검토했으나 버린 대안
  ("충돌이 드문 특성상 비관적 락 대비 유리하다 판단")
- 이 선택이 틀리게 되는 조건

---

## 6. 미확인 — 확인 전 기재 금지

- YORR 게임별 Redis 스토어 담당자
- YORR WebSocket 세션 관리와 인증 연동 지점 상세
- IBK 슈퍼앱 "디자인 시스템 정의" 여부
  (`src/data/project.js`에 있으나 원본에 근거 없음 → 사용자 확인 필요)
- yorr-arcade 게임 2종의 정확한 명칭
- 주사위 위조 방지(VALIDATION) 담당자
- **발표자료(`docs/YORR 최종 발표.pdf`) 페이지 번호 매핑** —
  `yorr-evidence.md`의 페이지 지정이 후반부에서 실제와 어긋난다.
  페이지 번호로 사용 가능/금지를 판단하지 말고 산문 설명만 사용한다 (아래 참조)
- **냐암냐암 DBMS가 MySQL인지** — 발표자료는 "데이터베이스"로만 표기한다.
  확인 전까지 **「PHP + 관계형 DB」로만 쓰고 「MySQL」로 쓰지 않는다** (§2 참조)

### 출처 구분 — 「발표자료 확인」과 「본인 확인」은 다르다

원본 문서로 대조된 것과 사용자 진술로 확정된 것을 섞지 않는다.
후자는 재검증할 원본이 없으므로 **범위를 넓히는 방향으로 바꾸지 않는다.**

| 항목 | 출처 |
|---|---|
| 무신사 6인 팀 / 전 수치 / 분류 기준 / EDA 분포 | **발표자료** (`ML_project_최종2.pdf`) |
| 무신사 본인 담당 범위 (크롤러 구현·EDA / 모델링 공동) | **본인 확인** — 역할 슬라이드 없음 |
| 무신사 피처 2위(가격) · DevTools Network 서술 | **본인 확인** — 차트·스크린샷 이미지 |
| 냐암냐암 3인 팀 / PHP / 기능 / 역할 항목 | **발표자료** (`D조_발표_6월7일.pdf`) |
| 냐암냐암 조장 · 역할↔이름 매핑 | **본인 확인** — 표기 없음 |
| 냐암냐암 MySQL | **미확인** ← 위 목록 |

> **해소됨 (2026-08-27)** — coturn TURN 서버 연동 담당 범위.
> `docs/yorr-evidence.md`의 「음성을 서버에 태우지 않은 이유」 의사결정 블록과
> `yorr-architecture.svg`가 본인 담당으로 확인. §2·§4에 반영했다.

> **해소됨 (2026-08-27)** — 무신사 프로젝트 팀 규모·정확도·분류 방식·EDA 분포,
> 냐암냐암 팀 규모·담당·스택. `docs/ML_project_최종2.pdf`·`docs/D조_발표_6월7일.pdf`
> **원문 대조로 확정**했다 (§2 반영). **두 PDF는 `docs/`에 있다** — §0 참조.
> 대조 과정에서 2진 정확도 근거 페이지(48p→50p), EDA 편중 축(1점→0점),
> 3진 분류 기준 표기를 정정했다.

> **해소됨 (2026-08-27)** — 무신사 본인 담당 범위. **본인 확인**으로 확정:
> 데이터 수집은 크롤러 코드를 직접 작성, EDA는 본인 수행,
> **모델링은 6인 공동**. 기존 "데이터 수집·EDA·모델링 담당" 표기가 모델링을
> 실제보다 넓게 잡고 있어 §2와 전 산출물을 정정했다.

> **해소·정정됨 (2026-09-03)** — SSAFY 수상 2건의 명칭·순위·분모.
> **상장 실물 확인**으로 확정했다. 상장 제목은 두 건 모두 「우수상」이고
> 순위는 상장 **내용란의 부가 정보**다.
>
> | 구분 | 상장 제목 | 내용란 | 수여일 | 증서번호 |
> |---|---|---|---|---|
> | 공통 (YORR) | 우수상 | [2학기]공통 프로젝트 서울 4반 1등 | 2026.08.10 | 2026-45-050195 |
> | 관통 (TripCrew) | 1학기 프로젝트 우수상 | 1학기 관통 프로젝트 서울11반 2등 | 2026.06.26 | 2026-45-050066 |
>
> 기존 표기 「최우수상」·「공통 프로젝트 최종 1위」는 상장에 없는 표현이라
> 삭제했고, 「7팀 중 2위」의 분모도 상장에 기재가 없어 삭제했다.
> §6 「SSAFY 공통 프로젝트 수상 정확한 명칭 및 분모」 항목을 이로써 닫는다.

> **정정됨 (2026-08-28)** — YORR 소셜 로그인. 두 가지가 함께 틀려 있었다.
> 제공자가 Google 단독으로 적혀 있었고(실제로는 Google / Kakao),
> §4가 이를 본인 담당으로 기재하고 있었다.
> YORR 레포 auth 패키지 커밋 로그에 이정현 커밋이 없고 전부 팀원
> (고용훈 · yonghun) 작업으로 확인돼 §4에서 삭제하고 「담당 아님」 표로 옮겼다.
> TripCrew 의 Kakao / Naver 소셜 로그인은 별개이며 그대로 유지한다.
